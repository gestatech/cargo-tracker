...
public class GraphTraversalResource {
    ...
    public List<TransitPath> findShortestPath(@NotNull @Size(min = 5, max = 5) @QueryParam("origin") String originUnLocode, @NotNull @Size(min = 5, max = 5) @QueryParam("destination") String destinationUnLocode, @QueryParam("deadline") String deadline) {
        Date date = nextDate(new Date());
        List<String> allVertices = graphRepository.listLocations();
        ...
        for (int i = 0; i < candidateCount; i++) {
            ...
            transitEdges.add(new TransitEdge(graphRepository.getVoyageNumber(originUnLocode, firstLegTo), originUnLocode, firstLegTo, fromDate, toDate));
            ...
            for (int j = 0; j < allVertices.size() - 1; j++) {
                ...
                date = nextDate(toDate);
                transitEdges.add(new TransitEdge(graphRepository.getVoyageNumber(current, next), current, next, fromDate, toDate));
            }

            String lastLegFrom = allVertices.get(allVertices.size() - 1);
            fromDate = nextDate(date);
            toDate = nextDate(fromDate);
            transitEdges.add(new TransitEdge(graphRepository.getVoyageNumber(lastLegFrom, destinationUnLocode), lastLegFrom, destinationUnLocode, fromDate, toDate));
